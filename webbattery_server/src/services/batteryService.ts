...
export async function saveBatteryData(data: BatteryData): Promise<number> {
    ...
    try {
        ...
        // GET_for_Tesla: RAW 数据 (JSON arrays)
        const rawR2Json = (data as any).rawR2 ? JSON.stringify((data as any).rawR2) : null;
        const rawR3Json = (data as any).rawR3 ? JSON.stringify((data as any).rawR3) : null;

        const result = await db.run(
            `INSERT INTO battery_data 
       (device_number, mac, ip_prefix, device_address, r_ohm, r_sei, r_ct, voltage, test_type, dataready, timestamp, raw_r2, raw_r3)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
// Update RAW_R2 and RAW_R3 for the latest battery data record of a given MAC/IP+Addr
export async function updateLatestBatteryRawData(mac: string, rawR2: number[], rawR3: number[]): Promise<boolean> {
    const db = getDatabase();
    try {
        const rawR2Json = rawR2 ? JSON.stringify(rawR2) : null;
        const rawR3Json = rawR3 ? JSON.stringify(rawR3) : null;

        // 获取最新一条记录的ID
        const latestRow = await db.get(
            `SELECT id FROM battery_data WHERE mac = ? ORDER BY timestamp DESC LIMIT 1`,
            mac
        );

        if (latestRow) {
            await db.run(
                `UPDATE battery_data SET raw_r2 = ?, raw_r3 = ? WHERE id = ?`,
                rawR2Json,
                rawR3Json,
                latestRow.id
            );
        ...
        console.error('Error updating RAW battery data:', error);
        return false;
    }
}
...
export const exportDataToTXT = async (
    ...
    try {
        ...
        // 创建表格头部信息
        let txtContent = `电池阻抗测试上位机RS485 - 数据导出报告\r\n`;
        txtContent += `=========================================================\r\n`;
...
// 导出数据为Excel（xlsx），按测试类型及UID分Sheet，RAW R2/R3分sheet存储
export const exportDataToExcelByIp = async (
    ip: string,
    deviceAddress?: string,
    singleTestData?: Record<string, SingleTestResult[]>
): Promise<Buffer> => {
    try {
        const data = await getBatteryDataByIp(ip, deviceAddress);
        ...
        const summaryColumns = [
            ...
            { header: 'OCV(mV)', key: 'voltage', width: 15 },
            { header: 'Rohm(μΩ)', key: 'r1', width: 15 },
            { header: 'Rsei(μΩ)', key: 'r2', width: 15 },
            { header: 'Rct(μΩ)', key: 'r3', width: 15 }
        ];
        ...
        // 添加32个点列，标为 Point1 到 Point32
        for (let i = 0; i < 32; i++) {
            rawColumns.push({ header: `Point${i + 1}`, key: `p${i}`, width: 10 });
        }

        const populateSheets = (sheetSuffix: string, rows: any[]) => {
            const sumSheetName = `Period${sheetSuffix}`;
            const r2SheetName = `RAW_Rsei${sheetSuffix}`;
            const r3SheetName = `RAW_Rct${sheetSuffix}`;

            // Summary Sheet
            const summarySheet = workbook.addWorksheet(sumSheetName);
            ...
            // RAW R2 Sheet
            const r2Sheet = workbook.addWorksheet(r2SheetName);
            r2Sheet.columns = rawColumns;
            r2Sheet.getRow(1).font = { bold: true };
            r2Sheet.getRow(1).alignment = { vertical: 'middle', horizontal: 'center' };

            // RAW R3 Sheet
            const r3Sheet = workbook.addWorksheet(r3SheetName);
            r3Sheet.columns = rawColumns;
            r3Sheet.getRow(1).font = { bold: true };
            r3Sheet.getRow(1).alignment = { vertical: 'middle', horizontal: 'center' };

            rows.forEach(row => {
                const rawR2Str = (row as any).raw_r2;
                const rawR3Str = (row as any).raw_r3;
                if (!rawR2Str && !rawR3Str) return;
                ...
                if (rawR2Str) {
                    try {
                        const r2Arr = JSON.parse(rawR2Str);
                        const rowData: any = { ip: ipText, addr: addrText, time: timeStr };
                        r2Arr.forEach((v: number, i: number) => { rowData[`p${i}`] = v; });
                        r2Sheet.addRow(rowData);
                    } catch (e) { /* ignore */ }
                }

                if (rawR3Str) {
                    try {
                        const r3Arr = JSON.parse(rawR3Str);
                        const rowData: any = { ip: ipText, addr: addrText, time: timeStr };
                        r3Arr.forEach((v: number, i: number) => { rowData[`p${i}`] = v; });
                        r3Sheet.addRow(rowData);
                    } catch (e) { /* ignore */ }
                }
            });
        };

        const singleSummaryColumns = [
            { header: '时间', key: 'time', width: 22 },
            { header: 'Rohm(μΩ)', key: 'r1', width: 15 },
            { header: 'Rsei(μΩ)', key: 'r2', width: 15 },
            { header: 'Rct(μΩ)', key: 'r3', width: 15 },
            { header: '电压(mV)', key: 'voltage', width: 15 }
        ];

        if (deviceAddress) {
            // 单个 UID 模式：无后缀，干净的 Summary, RAW_R2, RAW_R3
            populateSheets('', data);

            // 始终创建 Single sheet
            const singleSumSheet = workbook.addWorksheet('Single');
            singleSumSheet.columns = singleSummaryColumns;
            singleSumSheet.getRow(1).font = { bold: true };
            singleSumSheet.getRow(1).alignment = { vertical: 'middle', horizontal: 'center' };

            // 填充数据如果有的话
            const deviceKey = `${ip}_${deviceAddress}`;
            const results = singleTestData ? singleTestData[deviceKey] : undefined;
            if (results && results.length > 0) {
                results.forEach(row => {
                    const date = new Date(row.timestamp);
                    const timeStr = `${date.getFullYear()}/${String(date.getMonth() + 1).padStart(2, '0')}/${String(date.getDate()).padStart(2, '0')} ${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}:${String(date.getSeconds()).padStart(2, '0')}`;

                    singleSumSheet.addRow({
                        time: timeStr,
                        r1: row.r1,
                        r2: row.r2,
                        r3: row.r3,
            ...
            // 多个 UID 模式：按 UID 分组生成 sheets
            const dataByUid = new Map<string, any[]>();
            data.forEach(row => {
                const addrRaw = row.device_address || ((row.mac || '').split('_')[1] || '01');
                const addrText = String(addrRaw).padStart(2, '0');
                if (!dataByUid.has(addrText)) {
                    dataByUid.set(addrText, []);
                }
                dataByUid.get(addrText)!.push(row);
            });

            // 获取所有可能存在数据的 UIDs (包括 database 中的周期性数据和 in-memory 中的单次测试数据)
            const allUidsSet = new Set<string>(dataByUid.keys());
            if (singleTestData) {
                Object.keys(singleTestData).forEach(deviceKey => {
                    if (deviceKey.startsWith(ip + '_')) {
                        const uid = deviceKey.split('_')[1];
                        if (uid) {
                            allUidsSet.add(uid.padStart(2, '0'));
                        }
                    }
                });
            }
            ...
            // 获取排序后的 UIDs
            const sortedUids = Array.from(allUidsSet).sort();
            if (sortedUids.length === 0) {
                // 没有数据时创建一个空 sheet
                workbook.addWorksheet('No Data');
            } else {
                sortedUids.forEach(uid => {
                    const deviceData = dataByUid.get(uid) || [];
                    populateSheets(`_${uid}`, deviceData);

                    // 始终为该 UID 创建 Single_${uid} sheet
                    const singleSumSheet = workbook.addWorksheet(`Single_${uid}`);
                    singleSumSheet.columns = singleSummaryColumns;
                    singleSumSheet.getRow(1).font = { bold: true };
                    singleSumSheet.getRow(1).alignment = { vertical: 'middle', horizontal: 'center' };

                    // 填充单次测试数据
                    const deviceKey = `${ip}_${uid}`;
                    const results = singleTestData ? singleTestData[deviceKey] : undefined;
                    if (results && results.length > 0) {
                        results.forEach(row => {
                            const date = new Date(row.timestamp);
...
export const exportAllUidsZip = async (ip: string): Promise<Buffer> => {
    const zip = new AdmZip();

    for (let uid = 1; uid <= 12; uid++) {
        const deviceAddr = String(uid).padStart(2, '0');

        let singleData: Record<string, SingleTestResult[]> | undefined;
        try {
            const { singleTestResults } = await import('./pollingService');
            if (singleTestResults.size > 0) {
                const key = `${ip}_${deviceAddr}`;
                const val = singleTestResults.get(key);
                if (val) {
                    singleData = {};
                    singleData[key] = val;
                }
            ...
            console.warn(`[ExportAll] 获取单次测试结果失败:`, e);
        }

        try {
            const xlsxBuffer = await exportDataToExcelByIp(ip, deviceAddr, singleData);
            zip.addFile(`battery_data_${ip}_${deviceAddr}.xlsx`, xlsxBuffer as Buffer);
        } catch (e) {
            console.warn(`[ExportAll] UID ${uid} 无数据，跳过:`, (e as Error)?.message);
        }
    }
