...
export const REGISTER_MAP = {
  // 数据寄存器 (0x0000-0x0004)
  VOLTAGE: 0x0000,          // 电压 (mV)
  BAT1_R1: 0x0001,          // 电池1阻抗 R1 (μΩ)
  BAT1_R2: 0x0002,          // 电池1阻抗 R2 (μΩ)
  BAT1_R3: 0x0003,          // 电池1阻抗 R3 (μΩ)
  OFFLINE_COUNTER: 0x0004,   // 离线测试计数器
  
  // 状态寄存器 (0x0005)
  STATUS_REGISTER: 0x0005,  // 状态寄存器
  
  // 控制寄存器
  CONTROL_A: 0x0006,        // 控制寄存器A (F2测试)
  CONTROL_B: 0x0007,        // 控制寄存器B (F1周期测试) - 与CONTROL_CYCLE相同
  CONTROL_CYCLE: 0x0007     // 周期值写入 (启动周期测试)
} as const;
...
/**
 * 状态寄存器位定义 (0x0005) - 根据最新协议定义
 */
export const STATUS_BITS = {
  MEAS_ENABLE: 0x0001,      // bit0: 表示测试已启动
  MEAS_RUNNING: 0x0002,     // bit1: 当前测试正在执行中
  ALARM_DEV_OV: 0x0004,     // bit2: 检测设备供电异常
  ALARM_CELL1_OV: 0x0008,   // bit3: 电池电压超出上限
  ALARM_CELL1_UV: 0x0010,   // bit4: 电池电压低于下限
  COMM_TIMEOUT: 0x0020,     // bit5: 通讯超时故障
  FORCE_STOPPED: 0x0040,    // bit6: 被强行中止标志
  DATA_READY: 0x0080,       // bit7: F1采样数据更新标志
  COMM_ERROR: 0x0100,       // bit8: 写入命令错误标志
  ADDRESS_MASK: 0x1E00      // bit9-12: 地址寄存器 (0x0F << 9)
} as const;
...
export function parseStatusRegister(statusValue: number): {
  ...
  return {
    address: (statusValue & STATUS_BITS.ADDRESS_MASK) >> 9, // 地址在Bit9-12
...
export function parseBatteryData(values: number[]): {
  ...
  controlRegisterA?: number; // 控制寄存器A (保留兼容)
  controlRegisterB?: number; // 控制寄存器B (保留兼容)
  ...
  if (values.length > 0) {
    ...
    console.log(`0x0000 电压: ${values[0]}mV`);
  }

  // 0x0001-0x0003: Bat1 R1-R3
  if (values.length > 3) {
    data.r1 = { value: values[1], actual: values[1] };
    data.r2 = { value: values[2], actual: values[2] };
    data.r3 = { value: values[3], actual: values[3] };
    console.log(`Bat1: R1=${values[1]}, R2=${values[2]}, R3=${values[3]}`);
  }

  // 0x0005: 状态寄存器
  if (values.length > 5) {
    const statusValue = values[5];
    console.log(`0x0005 状态寄存器: 0x${statusValue.toString(16)}`);
  ...
  // 0x0006: 控制寄存器A
  if (values.length > 6) {
    data.controlRegisterA = values[6];
  }

  // 0x0007: 控制寄存器B
  if (values.length > 7) {
    data.controlRegisterB = values[7];
  }
