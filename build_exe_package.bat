...
if exist packages\exe\final_exe rmdir /s /q packages\exe\final_exe
mkdir single-exe
mkdir packages
mkdir packages\exe
mkdir packages\exe\final_exe
...
echo     "outputPath": "../packages/exe/final_exe", >> single-exe\package.json
...
REM 使用PKG打包为单个exe文件
echo 正在打包为单个exe文件...
call npx pkg . --targets node18-win-x64 --output ../packages/exe/final_exe/WebBattery.exe --compress GZip
if %ERRORLEVEL% NEQ 0 (
  echo PKG压缩打包失败，尝试无压缩模式...
  call npx pkg . --targets node18-win-x64 --output ../packages/exe/final_exe/WebBattery.exe
  if %ERRORLEVEL% NEQ 0 (
    echo PKG打包失败！
    echo 请检查：
    echo 1. 网络连接是否正常
...
REM 创建使用说明
echo WebBattery 监控系统 - 单文件版本 > packages\exe\final_exe\使用说明.txt
echo ===================================== >> packages\exe\final_exe\使用说明.txt
echo. >> packages\exe\final_exe\使用说明.txt
echo 使用方法: >> packages\exe\final_exe\使用说明.txt
echo 1. 双击 WebBattery.exe 启动程序 >> packages\exe\final_exe\使用说明.txt
echo 2. 等待服务器启动（约10-30秒） >> packages\exe\final_exe\使用说明.txt
echo 3. 系统会自动打开浏览器访问 http://localhost:8080 >> packages\exe\final_exe\使用说明.txt
echo 4. 如果浏览器未自动打开，请手动访问上述地址 >> packages\exe\final_exe\使用说明.txt
echo. >> packages\exe\final_exe\使用说明.txt
echo 功能特点: >> packages\exe\final_exe\使用说明.txt
echo - 单个exe文件，无需安装任何依赖 >> packages\exe\final_exe\使用说明.txt
echo - 自动打开浏览器，无需手动输入地址 >> packages\exe\final_exe\使用说明.txt
echo - 内置Node.js运行时，完全自包含 >> packages\exe\final_exe\使用说明.txt
echo - 智能检测服务器启动状态 >> packages\exe\final_exe\使用说明.txt
echo. >> packages\exe\final_exe\使用说明.txt
echo 注意事项: >> packages\exe\final_exe\使用说明.txt
echo - 首次启动需要时间解压文件（10-30秒） >> packages\exe\final_exe\使用说明.txt
echo - 确保8080端口未被其他程序占用 >> packages\exe\final_exe\使用说明.txt
echo - 如果防火墙提示，请选择允许访问 >> packages\exe\final_exe\使用说明.txt
echo - 按 Ctrl+C 可以停止服务器 >> packages\exe\final_exe\使用说明.txt
echo. >> packages\exe\final_exe\使用说明.txt
echo 故障排除: >> packages\exe\final_exe\使用说明.txt
echo - 如果浏览器未自动打开：手动访问 http://localhost:8080 >> packages\exe\final_exe\使用说明.txt
echo - 如果端口被占用：关闭占用8080端口的其他程序 >> packages\exe\final_exe\使用说明.txt
echo - 如果启动失败：检查是否有杀毒软件阻止运行 >> packages\exe\final_exe\使用说明.txt
echo. >> packages\exe\final_exe\使用说明.txt
echo 版本: 1.0.0 (单文件版) >> packages\exe\final_exe\使用说明.txt
echo 构建时间: %date% %time% >> packages\exe\final_exe\使用说明.txt
...
echo ========================================
echo.
if exist packages\exe\final_exe\WebBattery.exe (
  echo [成功] 成功创建单文件EXE: packages\exe\final_exe\WebBattery.exe
  echo.
  echo [使用方法]:
  ...
  echo.
  echo [输出目录]: packages\exe\final_exe\
) else (
  echo [错误] EXE文件创建失败！
  echo 请检查错误信息并重试
)
echo.
echo 详细说明请查看: packages\exe\final_exe\使用说明.txt
echo.
pause